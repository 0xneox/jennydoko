...
function generateTileClick(): Float32Array {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    // Woodblock sharp transient with pitched resonance
    const f1 = 920;
    const f2 = 1450;
    const env = Math.exp(-t * 65); // Fast exponential decay
    const bodyEnv = Math.exp(-t * 40);

    const click = Math.sin(2 * Math.PI * f1 * t) * 0.6 + Math.sin(2 * Math.PI * f2 * t) * 0.4;
    // Tiny subtle wooden knock noise in the first 5ms
...
function generatePuppyPlace(): Float32Array {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    // Puppy yip: two-part upward pitch sweep (550Hz -> 1050Hz -> 850Hz)
    let yip = 0;
    if (t < 0.18) {
      const yipEnv = Math.sin((t / 0.18) * Math.PI); // Smooth attack and release
      const freq = 550 + Math.sin((t / 0.18) * Math.PI * 0.5) * 500;
      const phase = 2 * Math.PI * freq * t;
      // Formants for puppy bark voice (fundamental + 2nd + 3rd harmonic)
      yip = (Math.sin(phase) * 0.6 + Math.sin(phase * 2) * 0.25 + Math.sin(phase * 3) * 0.15) * yipEnv * 0.6;
    }

    // Soft crystalline bell chime (1760Hz - A6 + 2637Hz - E7 harmonic)
    const chimeEnv = Math.exp(-t * 8.5); // Warm ringing decay
...
function generateLevelClear(): Float32Array {
  const duration = 3.0; // 3.0 seconds
  const numSamples = Math.floor(SAMPLE_RATE * duration);
  const samples = new Float32Array(numSamples);

  // Arpeggio notes in C major 9 (C4, E4, G4, B4, D5, E5, G5)
  const notes = [
    ...
    { freq: 783.99, time: 0.86 }, // G5 (sustained chord peak)
    { freq: 1046.50, time: 1.05 }, // C6 high bell top
  ];
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    for (const note of notes) {
      if (t >= note.time) {
        ...
        // Plucked acoustic string envelope (sharp plucking transient, warm exponential body)
        const env = Math.exp(-dt * 1.8);
        const f = note.freq;

        // Karplus/string harmonic blend
...
function generateUndoErase(): Float32Array {
  ...
  for (let i = 0; i < numSamples; i++) {
    const t = i / SAMPLE_RATE;
    // Friction sweep envelope (rise then soft taper)
    const env = Math.sin((t / duration) * Math.PI);
    // Filtered noise with downward friction sweep
...
// 5. Hint: Magical sparkle sound (cascading crystalline fairy bells)
function generateHintSparkle(): Float32Array {
  const duration = 0.5; // 500ms
  const numSamples = Math.floor(SAMPLE_RATE * duration);
  const samples = new Float32Array(numSamples);

  const sparkleFrequencies = [2093, 2637, 3136, 4186, 4698]; // C7, E7, G7, C8, D8
...
// Seamlessly looping 4-bar relaxing fingerpicked guitar and kalimba tines (~78 BPM, 4/4)
// Total 4 bars at 78 BPM = 4 * (4 * 60 / 78) = 12.307 seconds
function generateBgm(): Float32Array {
  const bpm = 78;
  const beatDuration = 60 / bpm; // ~0.769 sec per beat
  const totalBeats = 16; // 4 bars * 4 beats
  const duration = totalBeats * beatDuration;
  const numSamples = Math.floor(SAMPLE_RATE * duration);
  const samples = new Float32Array(numSamples);

  // 4 Chords: Cmaj7 (C-E-G-B) -> Am9 (A-C-E-G-B) -> Fmaj7 (F-A-C-E) -> G7sus4/G (G-D-F-B)
  // Bass line (plucked fingerstyle guitar root notes)
  const bassNotes = [
  ...
  // Kalimba / acoustic arpeggio melody (gentle high tines)
  const kalimbaNotes = [
    // Bar 1 (C)
    { beat: 0.5, freq: 523.25 }, // C5
    { beat: 1.5, freq: 659.25 }, // E5
    { beat: 2.0, freq: 783.99 }, // G5
    { beat: 3.0, freq: 987.77 }, // B5
    // Bar 2 (Am)
    { beat: 4.5, freq: 440.00 }, // A4
    { beat: 5.5, freq: 523.25 }, // C5
    { beat: 6.0, freq: 659.25 }, // E5
    { beat: 7.0, freq: 783.99 }, // G5
    // Bar 3 (F)
    { beat: 8.5, freq: 349.23 }, // F4
    { beat: 9.5, freq: 440.00 }, // A4
    { beat: 10.0, freq: 523.25 }, // C5
    { beat: 11.0, freq: 659.25 }, // E5
    // Bar 4 (G)
    { beat: 12.5, freq: 392.00 }, // G4
    { beat: 13.5, freq: 493.88 }, // B4
    { beat: 14.0, freq: 587.33 }, // D5
    { beat: 15.0, freq: 659.25 }, // E5 -> gentle resolve into loop
  ];
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    for (const tine of kalimbaNotes) {
      ...
      if (t >= noteTime) {
        ...
        const f = tine.freq;
        // Pure kalimba tines (fundamental + bell overtone)
        const tone =
    ...
    // Soft warm master compression
    samples[i] = Math.tanh(sampleVal * 1.4) * 0.55;
  }

  // Crossfade first and last 20ms to guarantee 100% seamless clickless loop
